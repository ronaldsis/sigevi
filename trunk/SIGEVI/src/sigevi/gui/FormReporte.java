...
public class FormReporte extends javax.swing.JInternalFrame {
    ...
    private Cliente getCliente(int codigo) {
        ...
        Object obj = null;
        try {
            obj = sqlMapClient.queryForList("getCliente", codigo);
        } catch (SQLException ex) {
            Logger.getLogger(sigevi.gui.FormReporte.class.getName()).log(Level.SEVERE, null, ex);
        }
        cliente = ((Cliente) obj);
        ...
        private void rptClienteTop() {
        //Cliente cliente = new Cliente();
        SqlMapClient sqlMapClient = SqlMapConfig.getSqlMap();
        Object obj = null;
        try {
            obj = sqlMapClient.queryForList("rptClienteTop");
        } catch (SQLException ex) {
            Logger.getLogger(sigevi.gui.FormReporte.class.getName()).log(Level.SEVERE, null, ex);
        }
        List<String> dato = new ArrayList<>();
        dato = (List) obj;
        System.out.println("xxxxxxxxxxxxx"+dato.getClass());         
    }
